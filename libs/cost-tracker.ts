...
export class CostTracker {
  ...
  private static async triggerAlert(alert: any, currentSpend: number) {
    ...
    console.log(`🚨 Cost Alert Triggered for user ${alert.userId}: $${currentSpend.toFixed(4)} exceeds threshold $${alert.threshold}`);
  }
  ...
  // Get global cost analytics (for admin dashboard)
  static async getGlobalCosts(period: 'daily' | 'monthly' = 'monthly', limit = 30) {
    const summaries = await prisma.costSummary.findMany({
      where: {
        period,
        userId: { not: null } // Exclude global summaries
      },
      orderBy: { date: 'desc' },
      take: limit,
      include: {
    ...
    const totalRevenue = summaries.reduce((sum, s) => sum + s.totalCost, 0);
    const avgCostPerUser = summaries.length > 0 ? totalRevenue / summaries.length : 0;
